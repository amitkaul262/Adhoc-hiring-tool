...
//
// Wrapped in React's cache() so calling this multiple times within the
// same request (the shared app shell layout AND the page itself both
// need it) only hits Supabase once, not twice.
export const getCurrentEmployee = cache(async function getCurrentEmployee() {
  ...
  if (error || !employee) {
    ...
    console.error("getCurrentEmployee: no employee_master match", {
      lookingUpEmail: user.email,
      supabaseError: error,
    });
