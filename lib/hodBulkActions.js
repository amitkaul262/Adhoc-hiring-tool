...
import { sendRequisitionDecisionEmail, sendVendorNeededEmail } from "@/lib/email";
...
export async function bulkApprove(hodEmail, prevState, formData) {
  ...
  // Each requisition's emails are fully independent of every other's, so
  // this runs them all concurrently instead of one requisition at a time
  // — bulk-approving 10 requisitions was waiting on 20 sequential email
  // sends (2 each) before the button could even report success.
  await Promise.all(
    updated.map(async (r) => {
      try {
      ...
      try {
        await sendVendorNeededEmail(r);
      } catch (e) {
        console.error("sendVendorNeededEmail failed for", r.requisition_id, e);
      }
    })
  );

  revalidatePath("/dashboard");
  return { error: null, success: true, approvedCount: updated.length };
}
