...
export async function fetchWorkerPaymentRows(requisitionId = null) {
  ...
  const [{ data: workers, error: workersError }, { data: vendors }, { data: attendanceRows }] = await Promise.all([
    query,
  ...
  // A genuine query failure (e.g. a column that doesn't exist yet because
  // a migration hasn't been run) must never look identical to "there's
  // legitimately no data here" — the first is a bug to surface loudly,
  // the second is a normal empty state. Callers check this explicitly
  // rather than just treating an empty rows array as "nothing to show."
  if (workersError) {
    console.error("fetchWorkerPaymentRows: query failed", { requisitionId, error: workersError });
    return { rows: [], vendors: vendors || [], error: workersError.message };
  }
  ...
  const rows = (workers || [])
    ...
    .map((w) => {
      ...
      // Each worker can be assigned to their own vendor (splitting one
      // requisition's headcount across multiple suppliers) — falls back
      // to the requisition-level vendor for backward compatibility with
      // existing single-vendor requisitions where only that was ever set.
      const effectiveVendorId = w.vendor_id || w.requisitions.vendor_id || null;
      const vendorName = effectiveVendorId ? vendorNameById[effectiveVendorId] || "Unknown vendor" : null;
      const gstPct = effectiveVendorId ? vendorGstById[effectiveVendorId] ?? 0 : 0;
  ...
  return { rows, vendors: vendors || [], error: null };
}
...
export function summarizeByRequisition(rows) {
  ...
  for (const r of rows) {
    ...
    if (r.vendor_id) {
      s.vendorIds.add(r.vendor_id);
      s.vendorNames.add(r.vendor_name);
      s.gstPercentages.add(r.gst_percentage ?? 0);
    }
  }
  ...
  return Object.values(byReq).map((s) => {
    ...
    // A requisition can now be split across multiple vendors — show the
    // single vendor's name when there's just one, "Multiple vendors"
    // when workers are split across more than one, and a mixed-rate
    // caveat on GST since different vendors can charge different rates.
    const isSplitAcrossVendors = s.vendorIds.size > 1;
    const singleVendorId = s.vendorIds.size === 1 ? [...s.vendorIds][0] : null;
    const vendorName = isSplitAcrossVendors ? "Multiple vendors" : s.vendorNames.size === 1 ? [...s.vendorNames][0] : null;
    // A single GST% only makes sense to show when there's exactly one
    // vendor (or all workers share the same rate even with different
    // vendors, an edge case that's still safe to collapse to one value).
    const singleGstPercentage = s.gstPercentages.size === 1 ? [...s.gstPercentages][0] : null;
    return {
      ...s,
      ...
      vendor_name: vendorName,
      gst_percentage: singleGstPercentage,
      is_split_across_vendors: isSplitAcrossVendors,
      vendorIds: undefined,
      vendorNames: undefined,
      gstPercentages: undefined,
    };
  });
}
