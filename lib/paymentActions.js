...
export async function saveWorkerPayments(actorEmail, prevState, formData) {
  ...
  if (currentError) {
    console.error("saveWorkerPayments: couldn't read current state before saving", currentError);
    return { error: "Couldn't verify current payment status. Please try again — nothing was saved." };
  }

  const currentById = Object.fromEntries((current || []).map((c) => [c.id, c]));

  // Runs all row updates in parallel instead of one at a time — a
  // requisition with a large roster (15-20+ workers) was turning every
  // payment save into that many sequential round trips before this.
  ...
  if (failed.length > 0) {
    console.error("saveWorkerPayments failed for", failed.map((f) => f.id), failed[0].error);
    return {
      error:
        failed.length === rows.length
          ? "Couldn't save any rows. Please try again."
          : `${rows.length - failed.length} of ${rows.length} rows saved — ${failed.length} failed and need to be retried.`,
    };
  }
  ...
  for (const reqId of affectedRequisitions) {
    ...
    if (allPaid && !wasAlreadyFullyPaid) {
      const { error: fpErr } = await admin.from("requisitions").update({ fully_paid_at: now }).eq("requisition_id", reqId);
      if (fpErr) console.error("saveWorkerPayments: failed to set fully_paid_at for", reqId, fpErr);
    } else if (!allPaid && wasAlreadyFullyPaid) {
      const { error: fpErr } = await admin.from("requisitions").update({ fully_paid_at: null }).eq("requisition_id", reqId);
      if (fpErr) console.error("saveWorkerPayments: failed to clear fully_paid_at for", reqId, fpErr);
    }
...
export async function uploadInvoiceFile(requisitionId, actorEmail, prevState, formData) {
  ...
  try {
    ...
    try {
      ...
      // Apps Script commonly returns an HTML error page (not JSON) when
      // something's wrong on its end — an expired/invalid deployment, a
      // permissions prompt it can't resolve headlessly, etc. This is a
      // meaningfully different failure than a genuine network error, so
      // it gets its own clear message and full logging.
      console.error("uploadInvoiceFile: relay returned non-JSON response", {
        status: res.status,
        bodyPreview: rawText.slice(0, 500),
      });
      return {
        error:
          "The upload service returned an unexpected response instead of a normal reply — the Apps Script deployment may need re-checking. See Vercel logs for the raw response.",
      };
    }
  } catch (e) {
    console.error("uploadInvoiceFile: relay call failed:", e.message, e);
    return { error: `Couldn't reach the upload service (${e.message || "network error"}). Check that APPS_SCRIPT_URL is correct and the Apps Script deployment is still live.` };
  }
